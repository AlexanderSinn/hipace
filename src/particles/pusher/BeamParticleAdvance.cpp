    ...
    omp::ParallelFor(
        ...
        [=] AMREX_GPU_DEVICE (int ip, auto depos_order, auto c_use_external_fields,
            ...
            for (; i < n_subcycles; i++) {
                ...
                if (do_z_push) zp += dt * amrex::Real::unchecked_sub( uz_next * gamma_next_inv, clight );
                ux = ux_next;
                uy = uy_next;
                uz = uz_next;
