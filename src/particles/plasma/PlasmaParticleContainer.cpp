                  ...
                  const amrex::Real background_density_SI)
{
    ...
    for (amrex::MFIter mfi_ion = MakeMFIter(0, DfltMfi); mfi_ion.isValid(); ++mfi_ion)
    {
        ...
        amrex::AnyCTO(
            ...
            const amrex::Real uz = ptd_ion.rdata(PlasmaIdx::uz_half_step)[ip];

            const amrex::Real Aabssqp = 0._rt; // TODO: add Aabssq contribution
        ...
        amrex::ParallelFor(num_ions,
            ...
            if(p_ion_mask[ip] != 0) {
                ...
                ptd_elec.rdata(PlasmaIdx::uz     )[pidx] = 0._rt;
                ...
                ptd_elec.rdata(PlasmaIdx::uz_half_step)[pidx] = 0._rt;
#ifdef HIPACE_USE_AB5_PUSH
#ifdef AMREX_USE_GPU
#pragma unroll
#endif
                 ...
                 const amrex::Real background_density_SI)
{
    ...
    for (amrex::MFIter mfi_ion = MakeMFIter(0, DfltMfi); mfi_ion.isValid(); ++mfi_ion)
    {
        ...
        amrex::AnyCTO(
            ...
            const amrex::Real Aabssqp = 0._rt; // 0.5_rt*amrex::abs(A*A); TODO: add Aabssq contribution
            const amrex::Real gamma_inv = plasma_gamma_inv(ux, uy, uz, Aabssqp);
            const amrex::Real gamma_psi = plasma_gamma_psi(gamma_inv, uz);
    ...
    for (PlasmaParticleIterator pti(*this); pti.isValid(); ++pti)
    {
        ...
        reduce_op.eval(
            ...
            [=] AMREX_GPU_DEVICE (int ip) -> ReduceTuple
            {
                ...
                const amrex::Real uz = ptd.rdata(PlasmaIdx::uy)[ip];
                ...
                const amrex::Real gamma = plasma_gamma(ux, uy, uz, Aabssqp);
                const amrex::Real gamma_inv = plasma_gamma_inv(ux, uy, uz, Aabssqp);
                const amrex::Real gamma_psi = plasma_gamma_psi(gamma_inv, uz);
                // Weight with quasi-static weighting factor
                const amrex::Real w = ptd.rdata(PlasmaIdx::w)[ip] * gamma_psi;
